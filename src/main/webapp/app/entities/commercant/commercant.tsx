  ...
  return (
    <div>
      <h2 id="commercant-heading" data-cy="CommercantHeading">
        <Translate contentKey="agriCycleApp.commercant.home.title">Commercants</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="agriCycleApp.commercant.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/commercant/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="agriCycleApp.commercant.home.createLabel">Create new Commercant</Translate>
          </Link>
        </div>
      </h2>
        ...
        {commercantList && commercantList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="agriCycleApp.commercant.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('adresseCommerce')}>
                  <Translate contentKey="agriCycleApp.commercant.adresseCommerce">Adresse Commerce</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('adresseCommerce')} />
                </th>
                ...
                <th>
                  <Translate contentKey="agriCycleApp.commercant.utilisateur">Utilisateur</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {commercantList.map((commercant, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{commercant.adresseCommerce}</td>
                  <td>{commercant.moyenPaiement}</td>
                  <td>
                    {commercant.utilisateur ? (
                      <Link to={`/utilisateur/${commercant.utilisateur.id}`}>{commercant.utilisateur.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/commercant/${commercant.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="agriCycleApp.commercant.home.notFound">No Commercants found</Translate>
            </div>
          )
        )}
      </div>
